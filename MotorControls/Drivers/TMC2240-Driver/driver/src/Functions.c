...
/* Lookup table for integer square root function */
static const uint8_t sqrttable[256] = {
                           ...
                           uint8_t actualFilter, uint8_t maxFilter, int32_t *out)
{
    ...
    int64_t divisor;
    int64_t result;
    if ((akku == NULL) || (out == NULL)) {
        return TMC2240_ERROR_ARGUMENT;
    }
    if ((maxFilter > 31U) || (actualFilter > maxFilter)) {
        return TMC2240_ERROR_RANGE;
    }
    delta = ((int64_t)newValue - lastValue) *
            (INT64_C(1) << (maxFilter - actualFilter));
    if (((delta > 0) && (*akku > INT64_MAX - delta)) ||
        ((delta < 0) && (*akku < INT64_MIN - delta))) {
        return TMC2240_ERROR_RANGE;
    }
    next = *akku + delta;
    divisor = INT64_C(1) << maxFilter;
    result = next / divisor;
    if ((next < 0) && ((next % divisor) != 0)) {
        --result;
    }
